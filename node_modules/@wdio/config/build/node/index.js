...
function applyHeadlessFlag(caps, headless) {
  const target = caps.alwaysMatch || caps;
  const browser = (target.browserName || "").toLowerCase();
  if (!browser && !("alwaysMatch" in caps)) {
    for (const key of Object.keys(caps)) {
      const val = caps[key];
      if (val && typeof val === "object" && val.browserName) {
        applyHeadlessFlag(val, headless);
      }
    }
    return caps;
  }
  if (browser === "chrome" || browser === "chromium") {
    const opts = target["goog:chromeOptions"] || {};
    const args = opts.args || [];
    const alreadyHasHeadless = args.some((a) => a === "--headless" || a === "headless" || a.startsWith("--headless="));
    target["goog:chromeOptions"] = {
      ...opts,
      args: headless ? Array.from(/* @__PURE__ */ new Set([...args, ...alreadyHasHeadless ? [] : ["--headless"], "--disable-gpu"])) : args.filter((a) => a !== "--headless" && a !== "headless" && !a.startsWith("--headless="))
    };
  } else if (browser === "firefox") {
    const opts = target["moz:firefoxOptions"] || {};
    const args = opts.args || [];
    target["moz:firefoxOptions"] = {
      ...opts,
      args: headless ? Array.from(/* @__PURE__ */ new Set([...args, "-headless"])) : args.filter((a) => a !== "-headless" && a !== "--headless")
    };
  } else if (browser === "microsoftedge" || browser === "msedge" || browser === "edge") {
    const opts = target["ms:edgeOptions"] || {};
    const args = opts.args || [];
    target["ms:edgeOptions"] = {
      ...opts,
      args: headless ? Array.from(/* @__PURE__ */ new Set([...args, "--headless"])) : args.filter((a) => a !== "--headless" && a !== "headless" && !a.startsWith("--headless="))
    };
  }
  return caps;
}
    ...
    if (typeof this._initialConfig.headless === "boolean") {
      ...
      if (Array.isArray(this._capabilities)) {
        for (const cap of this._capabilities) {
          applyHeadlessFlag(cap, headless);
        }
      } else if (this._capabilities && typeof this._capabilities === "object") {
        applyHeadlessFlag(this._capabilities, headless);
      }
    }
